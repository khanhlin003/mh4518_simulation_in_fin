...
def clean_options_df(options_data: pd.DataFrame, curr_date: pd.Timestamp):
    try:
        ...
        options_data = options_data[
            (options_data["Mid"] > 0) & (options_data["IVM"] > 0)
        ...
        for csv_file in os.listdir(date_path):
            ...
            try:
                ...
                if not cleaned_data.empty:
                    ...
                    print(
                        f"No valid data in '{csv_file}' for date '{date_folder}'. Skipping save."
                    )
            except Exception as e:
                print(
                    f"Error processing file '{csv_path}' for date '{date_folder}': {e}"
                )
